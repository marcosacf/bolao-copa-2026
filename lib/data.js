  ...
  try {
    ...
    FASES_MM.forEach(k => {
      (MM_BRACKET[k] || []).forEach(m => {
        ...
        if (!userMMByMatchup[key]) userMMByMatchup[key] = { pal: userPal, u1, u2, fase: k };
      });
    });
    FASES_MM.forEach(k => {
      (MM_BRACKET[k] || []).forEach(m => {
        ...
        const ue = userMMByMatchup[key]; if (!ue) return;
